        ...
        for sequence in range(no_sequences):
            ...
            try:
                for frame_num in range(sequence_length):
                    res = np.load(os.path.join(DATA_PATH, action, str(sequence), video_nome.replace(".mp4", ""), "keypoints", "{}.npy".format(frame_num)))
                    window.append(res)
                sequences.append(window)
                labels.append(label_map[action])
            except FileNotFoundError:
                # Se o arquivo não for encontrado, trate a exceção aqui
...
## LAYER Dense 
model.add(LSTM(64, return_sequences=True, activation='relu', input_shape=(23,1662))) ##brincar com os valores
